...
export default function TieredMultipleChoiceView(props) {
  ...
  const onSubmit =() => {
    ...
    if(lists.length === 0 || dropdownLabels.length == selectedDropdownLists.length ) {
      ...
      setShowError(true);
      dispatch(showNotification({type:'success' , message: Strings.Complete_All_Fields, buttonText: 'Ok'  }));
    }    
  }
  

  return (
    <View style={{alignSelf:'stretch', marginHorizontal:10, marginTop:7, marginBottom:7}}>

      {
        dropdownLabels.map((element , index) => {
            return <TieredMultipleChoiceInput
            ...
            header={element} lists={getLists(index)} />
        })
