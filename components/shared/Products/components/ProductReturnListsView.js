...
export default function ProductReturnListsView(props) {
    ...
    const renderItem = (item, index , groupName) => {
        return (
            <View>
                {
                    index == 0 &&
        ...
        return (
            <View>
                <View style={{backgroundColor: Colors.primaryColor + '30', padding:5}}>
                    <AppText size="medium" title={name} color={whiteLabel().blackColor} style={{zIndex:999}}></AppText>
                </View>
                <View style={{flexDirection:'column'}}>
                    <View style={{flexDirection:'row', marginTop:5,marginBottom:5}}>
                        ...
                        <View style={{flex:1, marginRight:25}}>
                            <AppText type="primaryRegular"  title={'Qty'} color={whiteLabel().mainText} ></AppText>
                        </View>                    
                    </View>
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView style={{marginTop:10}}> 
                {
                    groups.map((element) => {                                                                    
                        var subItems = lists.filter((item) => item.productReturn === element.value);                        
                        return subItems.map((item , index) => {
                            return renderItem( item, index , element.label)
                        })
                    })
                }
