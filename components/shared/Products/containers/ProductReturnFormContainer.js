...
const ProductReturnFormContainer = React.forwardRef((props, ref) => {
    ...
    const changedSelectedProducts = (item , type) => {  
        if(type === "add"){        
            var check = selectedLists.find(element => element.product_id === item.product_id && element.productReturn === item.productReturn);
            if(check != undefined){
                var tmp = selectedLists.filter(element => !(element.product_id === item.product_id && element.productReturn === item.productReturn));
    ...
    const onSave = () => {        
        props.onSave({type: Constants.actionType.ACTION_FORM_SUBMIT, value: selectedLists});        
    }
    ...
    return (
        <View style={{alignSelf:'stretch' , flex:1, marginBottom: Platform.OS == 'android' ? 0 : 30 }}>
            ...
            <ProductListsModal 
                title= {questionType === Constants.questionType.FORM_TYPE_PRODUCT_RETURN ? "Product Return List" : "Product List"}
                hideClear={true}
            ...
            <SubmitButton title="Record" style={{marginTop:10, marginHorizontal:10}} onSubmit={onSave} />
