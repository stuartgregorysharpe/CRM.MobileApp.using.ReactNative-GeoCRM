...
export function find(postData){
  ...
  return new Promise(function(resolve, reject) {
        ...
        GetRequest.call( UrlResource.Form.Leadfields,  postData).then( async(res) => {
...
const getCommonTitle =  async(client_id, business_unit_id) =>{
    ...
    const titleQueryRes = await ExecuteQuery(titleQuery, [client_id, business_unit_id]);
    var commonTitle = '';
...
const getData = async (lists , commonTitle) => {
    ...
    for(var i = 0; i < lists.length; i++) {
        ...
        try{
            ...
            if(element.field_type == 'dropdown_text'){
                const query = getDropdownTextOptionQuery(element.custom_master_field_id);
                const res = await ExecuteQuery(query, []); 
                var _lists =  res.rows ? res.rows : [];
                presetOptions = getPresetOptionData(_lists);
            }
    ...
    return {status: String.Success, accuracy_distance_measure: 'm' , component_title: commonTitle, custom_master_fields : tmp};
...
const getPresetOptionQuery = (custom_master_field_id) => {
    var sql  = `SELECT  preset_data FROM locations_field_presets WHERE custom_master_field_id = ` + custom_master_field_id + ` ORDER BY preset_data `;
    return sql;
}
...
const getDropdownTextOptionQuery = (custom_master_field_id) => {
    var sql  =  `SELECT preset_data  FROM locations_dropdown_input_presets WHERE custom_master_field_id = ` + custom_master_field_id + ` ORDER BY preset_data`;
    return sql;
}

const getTriggerFieldQuery = (trigger_field_id) => {
    var sql  = `SELECT  field_type FROM locations_custom_master_fields WHERE custom_master_field_id = ` + trigger_field_id;
    return sql;
}
...
const getTriggerFieldData = (lists , element) => {
    ...
    for(var i = 0; i < lists.length; i++){
        ...
        if(subElement.field_type == 'multiple' || subElement.field_type  == 'multi_select'){
            console.log("subElement.trigger_field_answer",element.trigger_field_answer)
            tmp = {
                type: 'dropdown' , 
                trigger_field_id: element.trigger_field_id , 
                trigger_condition: element.trigger_field_condition,
