        ...
        GetRequest.call( "locations/location-info",  postData).then( async(res) => {
            ...
            if(res.status == Strings.Success && res.isConnected){
                ...
                if(client_id && business_unit_id){
                    ...
                    let locationInfoLists = await fetchLocationDataFromDB( business_unit_id, client_id,  postData.location_id);
                    const response = await getLocationData(locationInfoLists , contactsLists);
...
const fetchLocationDataFromDB = async( business_unit_id, client_id, location_id) => {
    const query = generateLocationDataQuery();        
    const res = await ExecuteQuery(query, [ business_unit_id, client_id , location_id ]);
    var lists =  res.rows ? res.rows : [];           
    return lists;
}

const generateContactQuery = () => {
    var query = `SELECT contact_name, contact_cell, contact_email ` +
                    `FROM crm_contacts ` + 
                    `WHERE location_id = ? ` +
...
const getLocationData = async (lists , contactsLists) => {
    ...
    for(var i = 0; i < lists.length; i++) {
        ...
        tmp = {
            ...
            last_interaction: lastVisit != null && lastVisit.length > 0 ? lastVisit[0] : '',
            last_visit: element.last_visit,
            address: element.location_unit_type + "," + element.location_unit + "," + element.street_address + "," + element.suburb + "," + element.city + "," + element.pincode,
            coordinates:coordinate,
            current_stage_id: '',
            current_outcome_id: '',
            location_image: element.location_image != "" ? baseURL + "/common/assets/location_images/" + element.location_image : '',
