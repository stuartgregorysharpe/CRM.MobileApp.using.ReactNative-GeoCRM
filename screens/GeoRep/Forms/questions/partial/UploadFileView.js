...
import { View, Text , Modal ,TouchableWithoutFeedback , TouchableOpacity ,StyleSheet, Dimensions, Platform } from 'react-native'
...
import { selectPicker } from '../../../../../constants/Consts';
import { ScrollView } from 'react-native-gesture-handler';
import DocumentPicker from "react-native-document-picker";


export default function UploadFileView({visible , onClose ,onValueChanged , item}) {
            ...
            <View style={{marginBottom:10}} key={"upload_file" + index}>
                <View style={{flexDirection:'row' , marginBottom:10, marginTop:10}}>
                    <View style={{marginLeft:10 , marginRight:10}}>
                        <SvgIcon icon={ element && element.name && element.name.includes(".mp3") ? 'Video_Library' : 'File'} width='25px' height='25px' />
                    </View>
                    <View style={{flex:1, flexDirection:'row', justifyContent:'center' , alignItems:'center'}}>
                        <View style={{flex:1 , flexDirection:'column', marginRight:10}}>
                            <View>
                                <AppText size="big" style={{fontWeight:'500', marginBottom:3}} title={element.name}></AppText>
                            </View>
                            <View style={{height:5, width: getBarWidth() , backgroundColor:Colors.greyColor}}>
                                <View style={{height:5, width:  item.value && item.value.length - 1 === index ? getBarWidth() * progress/10 : getBarWidth()
                    ...
                    {
                      (progress == 10 || item.value.length - 1 != index) &&
                      <View style={{backgroundColor:whiteLabel().itemSelectedIconFill, width:23,height:23 , borderRadius:20, alignItems:'center', marginLeft:10, marginRight:10 }}>                    
                          <SvgIcon icon={'Yes_No_Button_Check'} width='20px' height='20px' />
                      </View>
                    }
                ...
                <View style={style.centeredView}>
                    <View style={style.modalView}>    
                        ...
                        <View style={styles.sliderHeader}>                 
                            <Text style={{fontSize:16,fontFamily:Fonts.primaryBold , color:Colors.blackColor, fontSize:16, flex:1 }} >Select you file upload:</Text>
                        ...
                        {
                            item && item.value && item.value.length > 0 && item.value.map((element, index) => {
