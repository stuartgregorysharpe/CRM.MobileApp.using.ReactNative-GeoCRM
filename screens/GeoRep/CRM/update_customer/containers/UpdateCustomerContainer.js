...
export default function UpdateCustomerContainer(props) {
    ...
    const onAdd = () => {
        ...
        for(let key of Object.keys(customMasterFields)){
            var item = originCustomMasterFields.find(item => item.custom_master_field_id === key);
            ...
            if(item){
                ...
                if(item.field_type === "dropdown_input"){                    
                    custom_master_post_data.push({...tmp, dropdown_value: value.value, value: value.secondValue});
                }else{
        ...
        postApiRequest("locations-info/location-info-update", postData).then((res) => {
    ...
    const checkChangedStatus = () => {
        ...
        for(let key of Object.keys(customMasterFields)){
            var item = originCustomMasterFields.find(item => item.custom_master_field_id === key);
            if(item != null && item.value != customMasterFields[key] && item.core_field_name === "location_name"){
    ...
    return (
        <View style={{alignSelf:'stretch' , flex:1}}>
            ...
            <UpdateCustomerView    
                ...
                {...props}
            />            
            <SubmitButton style={{marginHorizontal:10, marginTop:5, marginBottom: Platform.OS == 'android' ? 10 : 30}} title={"Update"} onSubmit={onAdd}/>
