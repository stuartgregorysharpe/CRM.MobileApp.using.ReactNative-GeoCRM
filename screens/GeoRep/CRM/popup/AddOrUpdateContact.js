...
export default function AddContact({
  ...
  const [surname, setSurname] = useState(
    contactInfo ? contactInfo?.contact_surname : '',
  );
  const [email, setEmail] = useState(
    contactInfo ? contactInfo?.contact_email : '',
  );
  const [mobile_number, setMobileNumber] = useState(
    contactInfo ? contactInfo?.contact_cell : '',
  );
  const [additional_number, setAdditionalNumber] = useState(
    contactInfo ? contactInfo?.additional_number : '',
  );
  ...
  const handleSubmit = () => {
    ...
    addEditLocationContact(payload, uuid.v4())
      .then(response => {
        setIsSuccess(true);
        setShowErrorAlert(false);
        if (pageType === 'add') {
          ...
          <ScrollView>
            ...
            <View style={styles.header}>
              <Title style={{fontFamily: Fonts.primaryBold}}>
                {pageType === 'add' ? 'Add ' : 'Update '}Contact
              </Title>
              <Button
                labelStyle={{
