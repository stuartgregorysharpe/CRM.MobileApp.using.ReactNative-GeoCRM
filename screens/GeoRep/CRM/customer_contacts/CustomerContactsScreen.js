...
export const CustomerContactsScreen = forwardRef((props, ref) => {
  ...
  const handleSubmit = () => {
    ...
    updateCustomerLocationFields(request, uuid.v4())
      .then(response => {
  ...
  const initPostData = res => {
    ...
    res.forEach(element => {
      if (element.field_type === 'dropdown_input') {
        ...
        tmp[tmp.length - 1].itemIndex = presetOptions.findIndex(
          x => x === element.dropdown_value,
        );
      } else {
        tmp.push({
          custom_master_field_id: element.custom_master_field_id,
        ...
        <View style={{padding: 5, flex: 1}}>
          {locationFields.map((field, key) => {
            if (
              ...
              return <View key={key}>{renderText(field, key)}</View>;
            }
          })}
